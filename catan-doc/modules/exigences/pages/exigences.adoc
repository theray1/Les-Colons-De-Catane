...
[NOTE]
====
Ce document s'adresse aux membres du groupe de ce projet, ainsi qu'à la personne chargée de son évaluation.
...
Le logiciel ne garde pas trace de l'évolution d'un joueur, du nombre de partie qu'il a joué, etc... et ne permet pas de différencier de niveaux de joueurs, par exemple.
...
Le serveur s'assure d'avoir des parties de disponible pour les clients se connectants.
...
. Le logiciel doit pouvoir permettre d'envisager l'implémentations des extensions de {project} dans le futur, notamment.
